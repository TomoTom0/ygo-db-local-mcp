...
async function main() {
    ...
    for (const match of sortedResults){
        if (match.type === 'cardId') {
            ...
            if (resultCount === 1) {
                ...
                if (actualName !== providedName) {
                    ...
                    result.processedText = result.processedText.substring(0, match.startIndex) + replacement + result.processedText.substring(match.startIndex + match.pattern.length);
                }
        ...
        if (resultCount === 1) {
            ...
            const candidatesStr = match.results.map((card)=>`\`${card.name}|${card.cardId}\``).join('_');
