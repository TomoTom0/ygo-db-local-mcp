...
function resolveOutputPath(outputPath, outputDir) {
...
async function saveOutput(outputPath, content) {
...
async function executeAndSave(cliArgs, outputPath, outputDir) {
...
const paramsSchema = {
    ...
    mode: z.enum([
        "exact",
        "partial"
    ]).optional().describe("Search mode: 'exact' (default) or 'partial' (substring match)"),
...
server.tool("search_cards", `Search Yu-Gi-Oh cards database. Available fields: name, ruby, cardId, text (card effect), attribute, race, monsterTypes, atk, def, levelValue, pendulumText, supplementInfo. Use 'text' for card effects. Supports wildcard (*) in name and text fields, and negative search: -"phrase" to exclude.`, paramsSchema, async ({ filter, cols, mode, includeRuby, flagAutoPend, flagAutoSupply, flagAutoRuby, flagAutoModify, flagAllowWild, flagNearly, outputPath, outputDir })=>{
    const args = [
        ...tsxArgs,
...
const bulkParamsSchema = {
    queries: z.array(z.object(querySchema)).min(1).max(50).describe("Array of search queries (max 50). Each query has the same structure as search_cards."),
...
server.tool("extract_and_search_cards", `Extract card name patterns from text and search for them. Supports {card-name} (flexible/wildcard), 《card-name》 (exact), and {{card-name|cardId}} (by ID).`, {
    ...
    const args = [
        extractAndSearchScript,
        text
    ];
    return executeAndSave(args, outputPath, outputDir);
});
// Judge and replace tool
server.tool("judge_and_replace_cards", `Extract card patterns, search, and replace them intelligently. Results: 1 match → {{name|id}}, multiple → {{` + "`query`_`name|id`_...}}, none → {{NOTFOUND_`query`}}. Warns if unprocessed patterns remain.", {
    text: z.string().describe("Text with card patterns: {flexible}, 《exact》. Already processed {{name|id}} patterns are preserved."),
    outputPath: z.string().optional().describe("Optional: Save results to file. Filename or absolute path."),
    outputDir: z.string().optional().describe("Optional: Directory to save output file.")
}, async ({ text, outputPath, outputDir })=>{
    const args = [
        judgeAndReplaceScript,
        text
    ];
    return executeAndSave(args, outputPath, outputDir);
});
// Format converter tool
server.tool("convert_file_formats", `Convert between JSON, JSONL, JSONC, and YAML formats. Supports multiple file conversions in one call.`, {
...
server.tool("search_faq", `Search Yu-Gi-Oh! FAQ database by FAQ ID, card ID, card name/spec, question text, or answer text. Returns FAQs with embedded card information for all cards mentioned in the Q&A.`, {
    ...
    format: z.enum([
        ...
        'tsv',
        'jsonl'
    ]).optional().describe("Output format (default: json)"),
