...
HTTP_CODE=$(curl -L -w "%{http_code}" -o "$DATA_DIR/cards-all.tsv" \
  "$RELEASE_URL/$VERSION/cards-all.tsv" 2>&1 | tail -n1)
...
HTTP_CODE=$(curl -L -w "%{http_code}" -o "$DATA_DIR/detail-all.tsv" \
  "$RELEASE_URL/$VERSION/detail-all.tsv" 2>&1 | tail -n1)
